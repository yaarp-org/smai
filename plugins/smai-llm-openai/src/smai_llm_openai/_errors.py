"""OpenAI SDK exception → :class:`LlmProviderError` translation.

Per ``07-plugin-interfaces.md`` §4.5. The ``openai`` SDK raises typed
subclasses of :class:`openai.APIStatusError` keyed by HTTP status:
...
Plus connection / timeout errors (subclasses of
:class:`openai.APIConnectionError` / :class:`openai.APITimeoutError`).

Classification primarily by HTTP status (``exc.status_code``) — that
...
def translate_sdk_error(exc: BaseException) -> LlmProviderError:
    """Map an :class:`openai.APIError` (or sub-class) to a canonical
