"""Opt-in live Anthropic API smoke test.

Runs only when ``ANTHROPIC_API_KEY`` is set in the environment, per
the Task 3.F5 brief: opt-in live-call test runs locally with real
...
The test exercises one ``messages.create`` round-trip + one tool-use
round-trip against the configured model (default:
``claude-haiku-4-5`` — the cheapest Claude tier; override via
``ANTHROPIC_LIVE_MODEL_ID``).
"""
...
_SKIP_REASON = "live Anthropic test — set ANTHROPIC_API_KEY to run"


@pytest.mark.credentialed
@pytest.mark.skipif("ANTHROPIC_API_KEY" not in os.environ, reason=_SKIP_REASON)
async def test_live_round_trip() -> None:
    model_id = os.environ.get("ANTHROPIC_LIVE_MODEL_ID", "claude-haiku-4-5")
    provider = AnthropicProvider(model_id=model_id)
