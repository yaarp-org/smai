"""Pytest configuration for smai-llm-anthropic tests.
...
mode (set on the workspace root) does not implicitly extend
``sys.path``; this conftest does so explicitly. Mirrors the
