...
`ArtifactStore` plugin: BYO-bucket S3 implementation. Backed by `boto3` +
`asyncio.to_thread`; SigV4 presigned URLs for `url_for`.
...
2. **Threaded-moto presigned-URL tests** (`tests/test_presigned_url.py`):
   spins up `moto.server.ThreadedMotoServer` (a real local HTTP endpoint)
   so the presigned URL can be fetched end-to-end. No credentials.
