...
1. Reads the base YAML for the role from the package-resource
   directory ``smai_agents/prompts/<role>/base.yaml``.
2. If ``variant_name`` is provided, reads the variant YAML from
   ``smai_agents/prompts/<role>/variants/<variant_name>.yaml``.
...
from smai_agents.prompts.schema import (
    PromptConfig,
    PromptLayer,
    PromptLayerKind,
...
# directory layout is:
#
#   smai_agents/prompts/<role>/base.yaml
#   smai_agents/prompts/<role>/variants/<variant_name>.yaml
#
_PROMPT_PACKAGE = "smai_agents.prompts"
