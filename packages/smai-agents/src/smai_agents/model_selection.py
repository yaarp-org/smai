...
1. ``SystemConfig`` override (per DEC-022 — UI-configurable in v1, lifted to
   v2 ``MetadataStore``-backed system config). The shape of the v2 system
   config is owned by Task 2.C3's ``RuntimeConfig``; this function takes
   the resolved override map directly via the ``overrides`` arg, so we are
   not yet binding to a specific config-loading pathway.

2. ``SMAI_MODEL_<ROLE_UPPER>`` env var, format ``"<provider>:<model_id>"``.
   The v2 shape; v1's env-var convention was ``MODEL_<ROLE>``.
...
def get_model_for_task(
    ...
    *,
    ...
    1. ``overrides[role]`` — the SystemConfig-driven override map. The
       Phase 2 caller passes ``None`` (the v2 ``RuntimeConfig`` shape
       lands in Task 2.C3); the kwarg is shipped now so the surface
       does not change when the override map is wired in.
