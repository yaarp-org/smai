"""SQLAlchemy 2.0 Core schema shared by the :class:`MetadataStore` plugins.

Per DEC-036 (substrate choice — SQLAlchemy 2.0 async Core; Alembic
migrations land in Task 3.H2), DEC-030 (SQL-shape commitment), DEC-033
(transition log, normalized cost tables), DEC-035 (cursor pagination,
lease semantics, caller-resolved pool→states).
...
Per Task 3.H2 the schema lives in ``smai-orchestrator`` (a sibling of
``entities.tracking``) so both reference :class:`MetadataStore` plugins
import from one place. Prior to 3.H2 the declarations lived inside
``smai-store-sqlite``; the move drops the cross-plugin import that
``smai-store-postgres`` was carrying.
"""
...
# Single ``MetaData`` instance shared by every plugin. Dialect variations
# are rendered at engine-bind time, not declaration time.
...
# === Retention policy table set (DEC-033 #1, #2) =============================
#
# Per Task 3.H2 / DEC-033: ``transition_log``, ``agent_sessions``, and
# ``run_costs`` grow unbounded — every state transition writes a row,
# every agent turn writes a row, every run completion writes a row.
# Production deployments graduate this with a retention sweep that
# deletes rows older than ``EngineConfig.retention_policies[<table>]``
# days. The map below names the columns the prune-pass orders on (each
# table's intrinsic timestamp; ``transition_log`` has ``occurred_at``,
# the others have a ``recorded_at`` / ``started_at``).
RETENTION_TABLES: dict[str, tuple[Table, Column[Any]]] = {
    "transition_log": (transition_log_table, transition_log_table.c["occurred_at"]),
    "agent_sessions": (agent_sessions_table, agent_sessions_table.c["started_at"]),
    "run_costs": (run_costs_table, run_costs_table.c["recorded_at"]),
}
