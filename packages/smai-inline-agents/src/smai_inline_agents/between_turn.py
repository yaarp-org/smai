...
from smai_inline_agents.truncation import should_truncate, truncate_messages

if TYPE_CHECKING:
    from smai_inline_agents.loop import AgentSession
...
async def maybe_run_supervisor_check(session: AgentSession) -> None:
    ...
    1. Resolves the supervisor :class:`smai_core.plugins.LlmProvider`
       ...
       is set.
    2. Builds :class:`smai_inline_agents.agents.supervisor.SupervisorInput`
       from the session's recent status snapshots + recent tool-call
       names + role / turn / budget.
    3. Calls :func:`smai_inline_agents.agents.supervisor.run_supervisor_check`.
    ...
    # PromptConfig, which transitively pulls smai_inline_agents.prompts; that
    ...
    # pay the import cost.
    from smai_inline_agents.agents.supervisor import (  # noqa: PLC0415
        SupervisorInput,
        run_supervisor_check,
    )
    from smai_inline_agents.structured_call import StructuredCallFailed  # noqa: PLC0415
