...
``smai_inline_agents/prompts/planner/`` are first-cut content per the brief's
...
from smai_inline_agents.agent_session_telemetry import (
    close_agent_session,
    make_progress_sink,
    open_agent_session,
)
...
# Test-only seam shape mirrors :data:`smai_inline_agents.agents.harness_builder.SessionRunner`.
...
def make_dispatch_planner(
    *,
    ...
    async def _dispatch(ctx: Any) -> Any:  # ctx: DispatchContext
        ...
        # ``smai_inline_agents.agents.harness_builder.make_dispatch_harness_build``
