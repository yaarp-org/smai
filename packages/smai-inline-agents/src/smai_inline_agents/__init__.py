...
* Standard tool inventory — file ops, search, list_files, execute,
  run_experiment — under :mod:`smai_inline_agents.std_tools`. Convenience
  helper :func:`build_standard_tool_registry`.
* Prompt-config surface (2.B2) — YAML-on-disk + Pydantic-in-process —
  under :mod:`smai_inline_agents.prompts`. Public entry points
  ...
  bound to :class:`smai_inline_agents.prompts.PromptConfig` (full layered
...
from smai_inline_agents.agents import (
...
from smai_inline_agents.cache import (
    DEFAULT_CACHE_CONFIG,
    SINGLE_CALL_CACHE_CONFIG,
    CacheConfig,
)
from smai_inline_agents.loop import (
    AgentLoopConfig,
    AgentOutcome,
    AgentSession,
    run_loop,
)
from smai_inline_agents.model_selection import (
    TASK_DEFAULTS,
    ModelSelectionError,
    TaskRole,
    get_model_for_task,
...
from smai_inline_agents.retry_context import (
    PREV_CONVERSATION_TRACE_FILENAME,
    load_retry_context,
)
from smai_inline_agents.schemas import (
    CodeReviewResult,
    ContextualVerdict,
    EntryRanking,
    Finding,
...
from smai_inline_agents.std_tools import (
    EDIT_FILE_TOOL_NAME,
...
from smai_inline_agents.structured_call import StructuredCallFailed, structured_call
