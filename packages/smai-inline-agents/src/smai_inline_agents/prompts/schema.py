...
* ``base`` — the shipped default for the role (in
  ``smai_inline_agents/prompts/<role>/base.yaml``). Every role has exactly one
  base. Updating the base is a code-shipped change.
* ``variant`` — a deployment-selected alternative for the role
  (``smai_inline_agents/prompts/<role>/variants/<name>.yaml``). Multiple
...
class StructuredOutputTool(BaseModel):
    ...
    ``"smai_inline_agents.schemas.code_review:CodeReviewResult"``) — resolved
    ...
    schema_module: str = Field(
        ...
        description=(
            ...
            "(e.g., 'smai_inline_agents.schemas.code_review:CodeReviewResult'). "
...
class PromptLayer(BaseModel):
    ...
    Every YAML file under ``smai_inline_agents/prompts/`` deserializes to one
...
class PromptConfig(BaseModel):
    ...
    :func:`smai_inline_agents.prompts.load_prompt_config`; never written to
