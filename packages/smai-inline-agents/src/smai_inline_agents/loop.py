...
:class:`smai_inline_agents.tools.Tool` handlers and in
:mod:`smai_inline_agents.between_turn`.
"""
...
from smai_inline_agents.cache import DEFAULT_CACHE_CONFIG
from smai_inline_agents.model_selection import TaskRole
from smai_inline_agents.tools import (
    FINISH_TOOL_NAME,
    ToolContext,
    ToolRegistry,
)
from smai_inline_agents.truncation import TruncationPolicy
...
class AgentSession(BaseModel):
    ...
    """Path under which :func:`smai_inline_agents.between_turn.write_status`
    ...
    """Path under which :func:`smai_inline_agents.between_turn.maybe_check_supervisor_nudge`
    ...
    (the key :func:`smai_inline_agents.retry_context.load_retry_context`
    ...
    :func:`smai_inline_agents.between_turn.write_status` on the same cadence as
    ...
    between-turn :func:`smai_inline_agents.between_turn.write_status`
    produced. Trimmed to
    ...
    """Set to ``True`` by
    :func:`smai_inline_agents.between_turn.maybe_run_supervisor_check` when
...
async def run_loop(session: AgentSession) -> AgentOutcome:
    ...
    :mod:`smai_inline_agents.between_turn`; the import is local to break the
...
async def _run_turn_loop(session: AgentSession) -> AgentOutcome:
    ...
    from smai_inline_agents import between_turn  # noqa: PLC0415
